...
function ClientServerOperationsHandler(ch, m) {
	...
	// stick a reference to "this" into the ajax client, so that the
	// handleMessage
	// function can access the push client - its "this" is an XMLHttpRequest
	// object
	...
	function getAjaxClient() {
		/*
		 * Gets the ajax client http://en.wikipedia.org/wiki/XMLHttpRequest
	...
	/**
	 * pass in a callback and a channel. the callback should take a string,
	...
	ClientServerOperationsHandler.prototype.createConnection = function() {
		...
		try {
			var params = escape("channel") + "=" + escape(this.channel) + "&"
					+ escape("userName") + "=" + escape($("#userName").val());
			var url = "/QA_iASK/login?" + params;
			this.ajax.onreadystatechange = handleMessage;
			this.ajax.open("GET", url, true); // true means async, which is
	...
	/**
	 * If any message to be send to the server call this method to send the
	 * message note :all the messages
	 */
	...
	function handleMessage() {
		...
		// 1 (Open) The object has been created, but the send method has not
		// been called.
		// 2 (Sent) The send method has been called. responseText is not
		// available. responseBody is not available.
		// 3 (Receiving) Some data has been received. responseText is not
		// available. responseBody is not available.
		// 4 (Loaded)
		try {
			...
			if (this.readyState == 0) {
				...
				// for chunked encoding, we get the newest version of the entire
				// response here,
				// rather than in readyState 4, which is more usual.
				if (this.status == 200) {
					// this.ClientServerOperationsHandler.onMessage("3/200/" +
					// this.responseText.substring(this.responseText.lastIndexOf("|")));
				...
				if (this.status == 200) {
					...
					this.ClientServerOperationsHandler.onMessage("4/"
							+ this.status + "/-");
...
function onMessageReceive(model) {
	// format of chat message is so split it as
	...
	case CHATMESSAGE:
		...
		$("#" + messageFrom).find("#chatHistory").val(
				oldMessages + "" + message);
	...
	case NEWUSER:
		...
		$(".onlineUsersUL")
				.append(
						"<li><img class='onlineGreen' src='../images/onlineGreen.png'><a style='float:left;' href='#'>"
								+ messages[1] + "</a></li>");
