...
<%@page import="com.algorithm.AllStageOfUser"%>
...
<body>
	...
	<br>
	<center>
		<h2 style="color: navy;">Answerer Rating wise:</h2>
		<br>
		<%
			List<Integer> list = DataAceesLayer.getRewardList();
			for (Integer i : list) {
				UserCommunityInfomation finalCommunityInfomation = DataAceesLayer.getUserCommunityInfomation(i);
		%>
		<h3 style="color: navy;"><%=finalCommunityInfomation.getFname() + " " + finalCommunityInfomation.getLname()%></h3>
		<br>
		<%
			}
		%>
		____________________________________________________________________________________________________________________________________________________
		<h2>Answerer Response Rate and Delay wize:</h2>
		<%
			List<Integer> finalList = AllStageOfUser.BestAnswerer(communityInfomation.getCid());
			for (Integer uids : finalList) {
				UserCommunityInfomation finalCommunityInfomation = DataAceesLayer.getUserCommunityInfomation(uids);
		%>
		<h3 style="color: navy;"><%=finalCommunityInfomation.getFname() + " " + finalCommunityInfomation.getLname()%></h3>
		<br>

		<%-- <form action="" method="post">
			<input type="hidden" name="uid" value="<%=finalCommunityInfomation.getUid()%>">
			<input type="hidden" name="cid" value="<%=finalCommunityInfomation.getCid()%>">
