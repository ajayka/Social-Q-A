...
<body>
					...
					<center style="margin-top: -58px;">
						...
						<%
							...
							if (buff1 != null) {
								...
								baos1.close();
								String b64a = javax.xml.bind.DatatypeConverter.printBase64Binary(imageInByteArray1);
						%>
						<br>
						<br> <a
							href="myProfile.jsp?uid=<%=request.getParameter("uid")%>"><img
							src="data:image/png;base64, <%=b64a%>" alt="Visruth.jpg not found"
	...
	<center>
		<%
			for (UserCommunityInfomation allUser : allCommunityInfomation) {
				if (uid != allUser.getUid()) {

					BufferedImage buff = DataAceesLayer.getImg(allUser.getUid());
					if (buff != null) {
		...
		<br> <img src="data:image/png;base64, <%=b64%>"
			alt="Visruth.jpg not found"
			style="width: 105px !important; margin-left: -40%; border-radius: 20px;" /> <br> <br>
		<%
			} else {
		%>
		<img src="images/w1.jpg" alt="" WIDTH="100px" HEIGHT="100px"
			BORDER="0" style="border-radius: 24px; margin-left: -236px;">
		<%
			}
		%>


		<a href="friendsProfile.jsp?uid=<%=allUser.getUid()%>"><i><b
				style="color: navy; font-size: 27px; margin-left: 113px;"><%=allUser.getFname() + " " + allUser.getLname()%></b></i></a>

		<br>
		<br>__________________________________________________________________________________________________________________________________<br>
		<%
			}
			}
		%>
